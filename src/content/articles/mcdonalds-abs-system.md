...
The ripple effect is immediate. Drink assembly takes longer, which means the presenter waits longer, which means window times increase, which means the car line backs up, which means the drive-[thru timer](/articles/taco-bell-drive-thru-timer/)—the number that corporate watches more closely than almost any other metric—starts climbing into unacceptable territory. A good shift manager will immediately call for a technician and reorganize labor to minimize the damage, but even the best-managed store loses significant speed when the ABS is down.
